...
本地文件requirements.txt，cd到文件所在位置，打开powershell安装
```
...
DATABASES = {
    "default": {
        ...
        "PORT": 6379,          # db port（使用redis则改成）
        "NAME": "proxy",       # 默认配置（不同改动localhost）
        "PASSWORD": ""         # db password（redis默认无）
...
* 启动:
redis数据库部分：
安装redis服务端和客户端，安装redis包
![redis](redis.png)


```shell
需要安装包：apscheduler（anaconda中找不到这个包，使用pip install ascheduler）
...
* 生产环境 Docker/docker-compose（可以不用，忽略）
...
def get_html():
    ...
    retry_count = 3
    proxy = get_proxy()
    print("当前使用 http://{}".format(proxy.decode('utf8')))
    while retry_count > 0:
        try:
            response = requests.get('https://bilibili.com/', proxies={"http": "http://{}".format(proxy.decode('utf8'))})
            # 使用代理访问
            return response.content ##最理想的返回值##
        except Exception:
            retry_count -= 1
            print("count=",retry_count)
    # 出错3次, 删除代理池中代理
    print("删除ip：http://{}".format(proxy.decode('utf8')))
    delete_proxy(proxy)
    get_html()##重新获取一个ip
...
<font color="red" size="7">后面可以不用看</font>
